<template>
  <div>
    ...
    <div class="icon-box" v-else>
      <div :style="{width:userIconWidth+'px',height:userIconHeight+'px',background:'no-repeat 0% 0%',backgroundImage:`url(${userIconSrc})`,backgroundSize:'100% 100%', borderRadius:'50%'}" alt=""/>
      <p>{{ userName }}</p>
    </div>
  </div>
</template>
